    ...
    try {
      ...
      for (let i of quedadas) {
        if (moment(i.date) < moment() && i.status < 2 && i.status !== 3) {
          ...
          // Crear notificacion
          let exist = (await Notification.query().where({ quedada: i._id.toString(), user_id: i.user_id, title: 'Finalizó tu quedada' }).first())
          if (!exist) {
            const notif = {
              visto: false,
              user_id: i.user_id,
            ...
            Notifications.sendSystemNotification({userId: i.user_id, title: 'Finalizó tu quedada!', message: `${i.name} ha finalizado` })
          }
        }
  ...
  async allQuedadasAdmin ({ auth, request, response, view }) {
    try {
      ...
      if(user.roles[0] === 1 || user.roles[0] === 3){
        let quedadas = (await Quedada.query().where({privacy: 'Público'}).fetch()).toJSON()
        response.send(quedadas)
      } else {
    ...
    try {
      ...
      for (let i = 0; i < quedadas.length; i++) {
        if (moment(quedadas[i].date) < moment() && quedadas[i].status < 2) {
          ...
          await Quedada.query().where('_id', quedadas[i]._id).update({ status: 2 })
          // Crear notificacion
          let exist = (await Notification.query().where({ quedada: quedadas[i]._id.toString(), user_id: quedadas[i].user_id, title: 'Finalizó tu quedada' }).first())
          if (!exist) {
            const notif = {
              visto: false,
              user_id: quedadas[i].user_id,
      ...
      const { participo } = userCreador
        ...
        if (participo) {
          ...
          if ( creados === 10 && participo + 1 === 5 ) {
            ...
            Notifications.sendSystemNotification({userId: userCreador._id, title: '¡Desbloqueaste una nueva medalla', message: 'Tu medalla te espera 😎'})
        }
        }
    }
